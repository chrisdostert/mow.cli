...
func TestBoolOptMatcher(t *testing.T) {
	forceOpt := &opt{names: []string{"-f", "--force"}, value: &BoolOpt{}}
	optMatcher := &optMatcher{
		theOne: forceOpt,
		optionsIdx: map[string]*opt{
			"-f":      forceOpt,
			"--force": forceOpt,
			"-g":      {names: []string{"-g"}, value: &BoolOpt{}},
			"-x":      {names: []string{"-x"}, value: &BoolOpt{}},
			"-y":      {names: []string{"-y"}, value: &BoolOpt{}},
		},
	}
	cases := []struct {
		args  []string
...
func TestOptMatcher(t *testing.T) {
	...
	opts := []*opt{
		{names: names, value: &StringOpt{}},
		{names: names, value: &IntOpt{}},
		{names: names, value: &StringsOpt{}},
		{names: names, value: &IntsOpt{}},
	}
		...
		for _, forceOpt := range opts {
			...
			optMatcher := &optMatcher{
				...
				optionsIdx: map[string]*opt{
					"-f":      forceOpt,
					"--force": forceOpt,
					"-a":      {names: []string{"-a"}, value: &BoolOpt{}},
				},
			}
...
func TestOptsMatcher(t *testing.T) {
	opts := optsMatcher{
		options: []*opt{
			{names: []string{"-f", "--force"}, value: &BoolOpt{}},
			{names: []string{"-g", "--green"}, value: &StringOpt{}},
		},
